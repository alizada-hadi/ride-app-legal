# Passenger Privacy Policy

Last Updated: May 10, 2026

## Introduction

Your privacy is important to us. This Privacy Policy explains how the Masir Ride App collects, uses, stores, and protects your information when you use our ride booking and transportation services.

By using the Masir Ride App, you agree to the collection and use of information in accordance with this Privacy Policy.

---

# Information We Collect

We may collect the following types of information when you use the Passenger App:

## Personal Information

* Full name
* Phone number
* Email address
* Profile photo (if provided)

## Location Information

We collect real-time location information to provide ride booking services, including:

* Current device location
* Pickup location
* Drop-off location
* Route and trip tracking during active rides

## Ride Information

We may collect information related to your rides, including:

* Ride history
* Driver information
* Trip routes
* Booking timestamps
* Ride status
* Ratings and feedback

## Device Information

We may automatically collect device-related information such as:

* Device type and model
* Operating system
* Device identifiers
* Mobile network information

---

# How We Use Your Information

We use collected information for the following purposes:

* To provide ride booking services
* To connect passengers with drivers
* To calculate fares and trip details
* To improve app performance and user experience
* To provide customer support
* To send ride updates and notifications
* To detect fraud and improve platform safety
* To comply with legal obligations

---

# Location Data

Location access is essential for the Masir ride App to function properly.

We use location information to:

* Detect your current location
* Help drivers find pickup and drop-off points
* Provide real-time ride tracking
* Improve route accuracy and ride safety

Location tracking may continue during active rides even if the app is running in the background.

---

# Notifications

We may send notifications related to:

* Ride requests
* Driver arrival updates
* Trip status
* Security alerts
* Promotional or service announcements

You can manage notification permissions through your device settings.

---

# Sharing of Information

We may share limited information in the following situations:

## With Drivers

Passenger information such as name, pickup location, and contact details may be shared with drivers to complete rides.

## Service Providers

We may use trusted third-party services for:

* Maps and navigation
* Cloud hosting
* Analytics
* Notifications

## Legal Requirements

We may disclose information if required by law, regulation, legal process, or governmental request.

We do not sell your personal information to third parties.

---

# Data Retention

We retain user information for as long as necessary to:

* Provide services
* Maintain ride history
* Resolve disputes
* Comply with legal obligations
* Improve platform safety and reliability

Users may request account deletion as described below.

---

# Account Deletion

Users may request deletion of their account and personal information by navigating to the profile page and under settings page, they can see the delete account clearly.

Some information may be retained for legal, security, fraud prevention, or regulatory purposes.

---

# Data Security

We take reasonable measures to protect your information, including:

* Secure servers
* Encrypted communication
* Restricted data access
* Authentication and authorization controls

However, no method of electronic storage or internet transmission is completely secure.

---

# Children's Privacy

The Masir Ride App is not intended for children under the age of 13.

We do not knowingly collect personal information from children.

---

# Third-Party Services

The Masir Ride App may use third-party services such as:

* Google Maps
* Firebase
* Push notification providers
* Analytics platforms
* Cloud infrastructure providers

These third-party services may collect information according to their own privacy policies.

---

# Changes to This Privacy Policy

We may update this Privacy Policy from time to time.

Any changes will be posted on this page with an updated revision date.

Continued use of the app after changes become effective constitutes acceptance of the updated policy.

---

# Contact Us

If you have questions or concerns regarding this Privacy Policy, please contact us.

Email: [support@Masir.app](mailto:support@harja.app)
